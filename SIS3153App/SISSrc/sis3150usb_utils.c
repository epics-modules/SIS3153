...
int usb_load_tigersharcs(HANDLE hDevice,  char* loaderfile_path)
{
    ...
    ULONG data ;
    ULONG addr ;
    ULONG req_nof_lwords ;
    ULONG got_nof_lwords ;
    ...
    while (loadcount<(count-1)) {  
      ...
      switch (loader_header_word) {
	     ...
	     case 0x0: {       // final init
			...
			if ((error = sis3150Usb_TsBus_Dma_Write(hDevice, addr, (ULONG*)&tempword[loadcount], req_nof_lwords, &got_nof_lwords)) != 0) { 
				return -1;
			}
	     ...
	     case 0x40000000: {       // write data to memory
			...
			if ((error = sis3150Usb_TsBus_Dma_Write(hDevice, addr+loader_format_addr, (ULONG*)&tempword[loadcount], req_nof_lwords, &got_nof_lwords)) != 0) { 
				return -1;
			}
	    ...
	    case 0x80000000: {       // clear memory
			...
			req_nof_lwords = loader_format_length;
			if ((error = sis3150Usb_TsBus_Dma_Write(hDevice, addr+loader_format_addr, (ULONG*)clear_pattern, req_nof_lwords, &got_nof_lwords)) != 0) { 
				return -1;
			}
			req_nof_lwords = 1;
